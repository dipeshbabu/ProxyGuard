...
def run_phase(registry_path: Path, output_root: Path) -> pd.DataFrame:
    expected = registry_path.with_suffix(".sha256").read_text(encoding="utf-8").split()[0]
    if _sha256(registry_path) != expected:
        raise ValueError("Cost-phase registry digest mismatch.")
        ...
        for ratio_value in settings["release_to_target_cost_ratios"]:
            ...
            plan = recommend_cost_normalized_audit(
                ...
                expected_direct_score_probability=float(
                    scenario["direct_score_probability"]
                ),
                expected_named_recognition_probability=float(
                    scenario["named_recognition_probability"]
                ),
    ...
    output_path = output_root / "cost_mode_phase.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Map cost-normalized mode choice.")
    parser.add_argument("--registry", default="registries/proxyguard_cost_mode_phase.json")
    parser.add_argument("--output-root", default="outputs/proxyguard_cost_mode_phase")
    args = parser.parse_args()
    print(run_phase(Path(args.registry), Path(args.output_root)).to_string(index=False))
